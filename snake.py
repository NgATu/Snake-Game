...
APPLE = pygame.transform.scale(
    APPLE_IMG, (FOOD_WIDTH - 3, FOOD_WIDTH)
)  # minus 3 to make the apple less "fat"
# Color
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
GREEN = (0, 200, 0)
...
def draw_screen(snake: list[Snake], food: pygame.Surface, score: int) -> None:
...
def draw_snake(snake: list[Snake]):
...
def get_move(head: Snake, keys_pressed) -> None:
...
def moving_same_direction(first_part: Snake, second_part: Snake) -> None:
    """Make the snake can't move the opposite direction"""
...
def move(body_part: Snake):
    """Use for body parts except the head"""
...
def head_move(snake: list[Snake]):
    """Use for the head"""
    if snake[0].direction == "left" and snake[0].square.x - SNAKE_WIDTH >= 0:
        ...
        snake[0].direction == "right"
        and snake[0].square.x + SNAKE_WIDTH + SNAKE_WIDTH <= WIDTH
    ):
        ...
        snake[0].direction == "down"
        and snake[0].square.y + SNAKE_WIDTH + SNAKE_WIDTH <= HEIGHT
    ):
...
def get_hit(snake: list[Snake]) -> None:
    for i in range(1, len(snake)):
        if snake[0].square.colliderect(snake[i].square):
            pygame.event.post(pygame.event.Event(HIT_YOURSELF))


def food_appear_in_snake(snake: list[Snake], food: pygame.Surface) -> bool:
    for body in snake:
...
def create_random_food(snake: list[Snake]) -> pygame.Surface:
    while True:  # while the food appear in the snake
        x_co = (
            random.randint(0, WIDTH / FOOD_WIDTH - 1) * FOOD_WIDTH
        )  # which in this case is food width
        y_co = random.randint(0, HEIGHT / FOOD_WIDTH - 1) * FOOD_WIDTH
        food = pygame.Rect(x_co, y_co, SNAKE_WIDTH, SNAKE_WIDTH)
        if food_appear_in_snake(snake, food):
            break
    return food


def eat_food(snake: list[Snake], food: pygame.Surface) -> bool:
    if snake[0].square.x == food.x and snake[0].square.y == food.y:
        return True
    return False


def add_part(snake: list[Snake]) -> tuple[int]:
...
def main():
    ...
    x_start, y_start = (
        SNAKE_WIDTH * ((WIDTH / 2) // SNAKE_WIDTH),
